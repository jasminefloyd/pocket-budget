export default function AIInsightsScreen({ budget, setViewMode }) {
  ...
  const entertainmentExpenses = budget.transactions
    .filter((t) => t.category.toLowerCase().includes("entertainment") && t.type === "expense")
    .reduce((sum, t) => sum + t.amount, 0);
  ...
  const insights = [
    {
      ...
      description:
        totalExpenses > 0
          ? foodExpenses > totalExpenses * 0.3
          ...
          : `No expenses yet — add transactions to see insights!`,
    },
    {
      icon: "✅",
      title: "Good Habit",
      description:
        totalExpenses > 0
          ? transportExpenses < totalExpenses * 0.1
    ...
    },
    {
      ...
      description:
        totalIncome > 0 && entertainmentExpenses > 0
          ? `If you reduce entertainment by 10%, you could save an additional $${(entertainmentExpenses * 0.1).toFixed(2)} monthly.`
          : totalIncome > 0
          ? `Add entertainment expenses to unlock targeted saving suggestions.`
